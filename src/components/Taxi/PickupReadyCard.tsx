...
type PickupReadyCardProps = {
  onTakeOrder: () => void
}

export function PickupReadyCard({ onTakeOrder }: PickupReadyCardProps) {
  const { pickup, destination, price, tariff, etaMinutes } = TAXI_DEMO_ORDER
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 z-30 rounded-t-[24px] bg-white px-4 pt-5 pb-[calc(16px+env(safe-area-inset-bottom,0px))] shadow-[0_-4px_24px_rgba(0,0,0,0.12)]">
      ...
      <h2 className="mb-1 text-[22px] font-bold text-[#212121]">Вы на месте</h2>
      <p className="mb-4 text-[14px] text-[#9E9E9E]">Заказ готов к выполнению</p>

      <div className="mb-5 flex gap-3 rounded-2xl bg-[#F7F7F7] px-4 py-3">
        ...
        <div className="shrink-0 text-right">
          ...
          <p className="mt-1 text-[13px] text-[#9E9E9E]">~{etaMinutes} мин</p>
        </div>
      </div>
