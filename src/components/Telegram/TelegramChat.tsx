...
import { IconChevronBack } from './TelegramIcons'
  ...
  return (
    <div className="relative flex h-dvh max-h-dvh w-full flex-col overflow-hidden bg-[#DCF8C6]">
      <header className="z-10 flex shrink-0 items-center gap-1 border-b border-[#dbdbdb] bg-white px-1 py-2">
        <Link to={MODE_SELECTION_PATH} className="ml-1 shrink-0" aria-label="Меню режимов">
          <img
        ...
        <Link to={MODE_SELECTION_PATH} className="flex items-center pr-0.5" aria-label="Назад">
          <IconChevronBack />
        </Link>
      </header>
